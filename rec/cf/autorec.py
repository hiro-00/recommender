...
class AutoRec():
    def __init__(self, input_size):
        ...
        self.layer2 = tf.nn.relu(tf.add(tf.matmul(layer1, w2), b2))

        original = tf.mul(self.input_layer, self.unrated_mask)
        self.encoded = tf.mul(self.layer2, self.unrated_mask)
        self.loss = tf.reduce_sum(tf.squared_difference(original, self.encoded))
        self.loss = tf.add(tf.reduce_sum(tf.squared_difference(original, self.encoded)), tf.mul(0.03,
    ...
    def print_loss(self, rating, rating_list):
        unrated_mask = np.zeros(rating.shape)
        unrated_mask[rating != -1] = 1
        unrated_mask[rating == -1] = 0
        loss = self.session.run(self.loss, feed_dict={self.input_layer: rating,
                                                    self.unrated_mask: unrated_mask})
        print(loss)
        layer2 = self.session.run(self.layer2, feed_dict={self.input_layer: rating,
                                                    self.unrated_mask: unrated_mask})
        encoded = layer2.copy()
        encoded[rating == -1] = -1
        print(mask_rmse(rating, encoded))
        print(list_rmse(rating_list, layer2))
        print("--")